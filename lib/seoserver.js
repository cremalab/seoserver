...
(function() {
  var $, SeoServer, args, defaultConfig, express, logentries, memjs, querystring, seoserver, _,
  ...
  SeoServer = (function() {
    ...
    SeoServer.prototype.start = function() {
      ...
      memjs = this.initMemjs();
      memjs.fail(function(error) {
        return console.log("Got memjs connection error: " + error);
      });
      memjs.done(function(connection) {
        return console.log("Connected to memjs.");
      });
      memjs.always(function() {
        _this.app = express();
    ...
    SeoServer.prototype.fetchPage = function(request, response) {
      ...
      if (this.memjsClient) {
        fetchDfd = this.fetchFrommemjs(request);
      } else {
        console.log("fetch from phantom")
    ...
    SeoServer.prototype.storeResponseInCache = function(request, headers, content) {
      ...
      if (headers.status >= 200 && (headers.status < 300 || ((_ref = headers.status) === 301 || _ref === 302))) {
        return this.memjsClient.set(key, content, function(err) {
          if (err) {
    ...
    SeoServer.prototype.buildKey = function(url) {
      return "" + this.config.memjs.key + ":" + url;
    };

    SeoServer.prototype.fetchFrommemjs = function(request) {
      ...
      this.memjsClient.get(key, function(error, cachedContent) {
        var headers, matches, phantomRequest;

        if (error) {
          return dfd.reject("memjs error: " + error);
        }
    ...
    SeoServer.prototype.fetchFromPhantom = function(url) {
      console.log("begin fetch from Phantom");
      ...
      timeout = setTimeout(function() {
        console.log("KILLED ON TIMEOUT")
        return phantom.kill();
      }, 30000);
      phantom.stdout.on('data', function(data) {
        console.log("Phantom has data")
      ...
      phantom.on('exit', function(code, b) {
    ...
    SeoServer.prototype.initMemjs = function() {
      var client, dfd, server,
        _this = this;

      console.log("Launching memjs client");
      dfd = $.Deferred();
      if (!this.config.memjs.enabled) {
        return dfd.reject('memjs is disabled');
      }
      _this.memjsClient = memjs.Client.create();
      return dfd.resolve();
      return dfd.promise();
    };
    ...
    SeoServer.prototype.logResponseStats = function(request, headers, time) {
      ...
      status = headers.memcached ? "memjs" : headers.status ? headers.status : "KILLED";
